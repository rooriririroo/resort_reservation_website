...
<body style="text-align:center;">
	<!-- 헤더 -->
	<h3 style="color:#6b639f; margin-top:50px; font-size:25px; font-weight:bold;">예약현황</h3>
...
<%
	...
	if(request.getParameter("resv_date")!=null && request.getParameter("room")!=null) {
		resv_date = new String(request.getParameter("resv_date").getBytes("8859_1"),"utf-8");
...
%>
	...
	<%
			...
			// 이름 가운데 별표처리
			String name1 = resv_arr[1][i].substring(0,1);
			String name2 = resv_arr[1][i].substring(resv_arr[1][i].length()-1);
			if(resv_arr[1][i].length()<3) { // 이름이 3글자 이상이 아니라면
				resv_arr[1][i] = name1+"*";
			} else {
				String star = "";
				for(int j=0; j<resv_arr[1][i].length()-2; j++) {
					star = star+"*";
				}
				resv_arr[1][i] = name1+star+name2;	
			}
	%>
				<td style="
				<%if((resv_date.equals(resv_arr[0][i].substring(0,10))) && (room == 1)){
					...
					<span style="color:gray;"><%=resv_arr[1][i] %></span>
				</td>
	<%
		} 
	%>
	...
	<%
			...
			// 이름 가운데 별표처리
			String name1 = resv_arr[2][i].substring(0,1);
			String name2 = resv_arr[2][i].substring(resv_arr[2][i].length()-1);
			if(resv_arr[2][i].length()<3) { // 이름이 3글자 이상이 아니라면
				resv_arr[2][i] = name1+"*";
			} else {
				String star = "";
				for(int j=0; j<resv_arr[2][i].length()-2; j++) {
					star = star+"*";
				}
				resv_arr[2][i] = name1+star+name2;
			}
	%>
				<td style="
				<%if((resv_date.equals(resv_arr[0][i].substring(0,10))) && (room == 2)){
					...
					<span style="color:gray;"><%=resv_arr[2][i] %></span>
				</td>
	<%
		} 
	%>
	...
	<%
			...
			// 이름 가운데 별표처리
			String name1 = resv_arr[3][i].substring(0,1);
			String name2 = resv_arr[3][i].substring(resv_arr[3][i].length()-1);
			if(resv_arr[3][i].length()<3) { // 이름이 3글자 이상이 아니라면
				resv_arr[3][i] = name1+"*";
			} else {
				String star = "";
				for(int j=0; j<resv_arr[3][i].length()-2; j++) {
					star = star+"*";
				}
				resv_arr[3][i] = name1+star+name2;
			}
	%>
				<td style="
				<%if((resv_date.equals(resv_arr[0][i].substring(0,10))) && (room == 3)){
					...
					<span style="color:gray;"><%=resv_arr[3][i] %></span>
				</td>
	<%
		} 
	%>
