...
<head>
	...
	<style>
		...
		.idTd {width:50px; padding:5px 5px 5px 5px;}
		.titleTd {width:450px; padding:5px 5px 5px 5px;}
		.dateTd {width:130px; padding:5px 5px 5px 5px;}
		.countTd {width:70px; padding:5px 5px 5px 5px;}
		.btn {background-color:mediumpurple; margin-top:15px; width:100px; color:#ffffff; 
			  height:30px; border:0; outline:0; border-radius:5px;}
...
<body style="text-align:center;">
	<!-- 헤더 -->
	<h3 style="color:#6b639f; margin-top:50px; font-size:25px; font-weight:bold;">이용 후기</h3>
...
<%	
	...
	try{
		...
		// 1. 전체 행 갯수 계산
		ResultSet rset = stmt.executeQuery("select * from postscript;"); // execute : 모든 쿼리 실행 가능
		while(rset.next()) {
			lineCntTotal++;
		}
		
		// 2. 행 출력
		rset = stmt.executeQuery("select * from postscript order by rootid desc, recnt, id;"); // execute : 모든 쿼리 실행 가능
		while(rset.next()) {		
			lineCnt++; // 행 번호 카운트
			...
			// 새 글이면 NEW 표시
			String baseDate = rset.getString(3);
			if(today.equals(rset.getString(3))) { 
				title = title+"  [NEW]";
			} 
			
			// 댓글 종류에 따라 화살표 갯수 다르게 표시
			String arrow = "";
			String space = "";
			String line = "";
			if(rset.getInt(7) != 0) { // 원글을 제외한 글에 대해
				for(int i=0; i<rset.getInt(7); i++) { // relevel 갯수 만큼
					space = space + "&nbsp;";
					line = line + "-";
				}
				title = space+line+">[Re] "+title;
			}
%>
		<tr>
			<td class="idTd" style="background-color:lavender;">
				<a href="postscript/postscript_view.jsp?id=<%=rset.getInt("id") %>"><%=rset.getInt("id") %></a>
			</td>
			<td class="titleTd" style="text-align:left; text-overflow:ellipsis; overflow:hidden; white-space:nowrap;">
				<a href="postscript/postscript_view.jsp?id=<%=rset.getInt("id") %> "><nobr><%=title %></nobr></a>
<%
	// 파일이 존재한다면 표시
...
%>				
			...
			<td class="countTd"><%=rset.getInt("count") %></td>
			<td class="dateTd"><%=rset.getString("date") %></td>
		</tr>
<%
		}
...
%>
	...
	<div style="text-align:center; margin-left:600px;">
		<input type="button" class="btn" value="작성" onclick="location.href='postscript/postscript_insert.jsp';"/>
	</div>
...
%>
         ...
         <a href='e_02.jsp?from=1&cnt=<%=cntPT%>'> &lt;&lt; </a>
<%
   } else {
%>
         <!--  특수문자 << -->
         <!-- 다른 페이지일 때는 이전 페이지바로 이동 -->
         <a href='e_02.jsp?from=<%=(fromPT/(cntPT*10)-1)*(cntPT*10)+1%>&cnt=<%=cntPT%>'> &lt;&lt; </a>
<%
   }
...
%>
      ...
      <a 
<% } %>
      href='e_02.jsp?from=<%=(i*cntPT-(cntPT-1))%>&cnt=<%=cntPT%>'><%=(i)%></a>
<%   
...
%>
      ...
      <a href='e_02.jsp?from=<%=(lineCntTotal-lineCntTotal%cntPT-cntPT-1)%>&cnt=<%=cntPT%>'> &gt;&gt; </a>
<% } else {
%>
      <a href='e_02.jsp?from=<%=(lineCntTotal-lineCntTotal%cntPT+1)%>&cnt=<%=cntPT%>'> &gt;&gt; </a>
<%
}
