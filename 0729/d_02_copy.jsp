...
<head>
	...
	function checkTel() {
		...
		if((telnum.length != 0 && !telnum.match(phoneRule1)&&!telnum.match(phoneRule2))
				|| (telnum.length != 0 && telnum.length<11)) {
			alert("올바른 연락처를 입력해주세요!");
			document.getElementById("telnum").value = "";
			$("#telnum").focus();
		} else if(telnum.length == 11) { // 하이픈 없이 입력한 경우 자동 추가
			var tel1 = telnum.substring(0,3);
			var tel2 = telnum.substring(3,7);
			var tel3 = telnum.substring(7,11);
			
			document.getElementById("telnum").value = tel1+"-"+tel2+"-"+tel3;
		}
	}
	// 날짜 차이 비교
...
%>
<%-- 				<td class="secondTd" >
					<input type="text" id="resv_date" name="resv_date" value="<%=resv_date %>" 
					style="width:680px; height:40px; border:none;" readonly />
				</td> --%>
				<td class="secondTd" >
					<input type="date" id="resv_date" name="resv_date" 
					style="width:680px; height:40px;" value="<%=resv_date %>" min="<%=today %>" max="<%=max_date %>" />
				</td>
<%		
...
%>
				<td class="secondTd" >
					<input type="date" id="resv_date" name="resv_date" 
					style="width:680px; height:40px;" min="<%=today %>" max="<%=max_date %>" />
				</td>
<%			} 
%>
			</tr>
