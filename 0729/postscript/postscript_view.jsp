...
<%
	...
	try{
		// postscript_list.jsp에서 get으로 id값 받아온다
		...
		PreparedStatement psmt = conn.prepareStatement("update postscript set count = count+1 where id = ?");
		psmt.setInt(1,selectId);
		psmt.executeUpdate();
		
		// 링크 클릭 시 해당 id의 모든 정보 가져온다
		PreparedStatement psmt2 = conn.prepareStatement("select * from postscript where id = ?");
		...
		if(!rset.next())
		{
			...
			out.print("<script>location.href='../e_02_list.jsp?from=1&cnt=10';</script>");
		}
		// 해당 번호의 글이 존재할 경우
		else{
...
%>
	...
	<table id="table01">
		...
		<tr>
			...
			<td class="secondTd" colspan=3><%=rset.getString(3) %></td>
		</tr>
		<tr>
			<td class="firstTd">조회수</td>
			<td class="secondTd" colspan=3><%=rset.getInt(5) %></td>
		</tr>
		<tr>
			<td class="firstTd">내용</td>
			<td class="secondTd" style="word-break:break-all; wrap:hard;" colspan=3><%=rset.getString(4) %></td>
		</tr>
<%
	// 파일이 있을 경우 표시
	if(rset.getString(9) != null) {
		...
		<tr>
			...
			<td class="secondTd" colspan=3>
				<a href="file_download.jsp?filename=<%=rset.getString(9) %>" >
					<i class="fa fa-paperclip"></i><%=rset.getString(9).substring(8) %>
				</a>
			</td>
		</tr>
		<%
...
%>	
		<tr>
			...
			<td class="secondTd" colspan=3><span style="font-size:14px;"><%=rset.getString(6) %></span></td>
		</tr>
			<tr>
				<td class="firstTd">댓글수준</td>
				<td style="width:250px; text-align:left; padding:5px 5px 5px 5px; border:2px solid lavender;" >
					<input type="text" id="level" name="level" style="width:50px; border:none;" value="<%=rset.getInt(7) %>" readonly /></td>
				<td class="firstTd"><span style="font-size:16px;">댓글순서</span></td>
				<td style="width:250px; text-align:left; padding:5px 5px 5px 5px; border:2px solid lavender;">
					<input type="text" id="order" name="order" style="width:50px; border:none;" value="<%=rset.getInt(8) %>" readonly />
				</td>
			</tr>
	</table>
...
%>
	<div style="text-align:center; margin-left:490px;">
		<input type="button" class="btn" value="목록" onclick="location.href='../e_02.jsp?from=1&cnt=10';"/> <!-- 목록으로 이동 -->
		<input type="button" class="btn" id="editBtn" value="수정" 
				onclick="location.href='postscript_update.jsp?id=<%=id %>';"/>
		<input type="button" class="btn" id="deleteBtn" value="삭제" onclick="checkDelete(<%=id %>);" />
		<input type="button" class="btn" id="replyBtn" value="댓글" 
				onclick="location.href='postscript_reply.jsp?id=<%=id %>';"/>
