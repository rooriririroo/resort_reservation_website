...
<%
	...
	try{
		// gongji_view.jsp에서 get으로 id값 받아온다
		...
		PreparedStatement pset = conn.prepareStatement("update gongji set count = count+1 where id = ?");
		pset.setInt(1,selectId);
		pset.executeUpdate();
		
		// 링크 클릭 시 해당글의 id에 해당하는 모든 정보를 가져온다
		PreparedStatement pset2 = conn.prepareStatement("select * from gongji where id = ?");
...
%>
	...
	<table id="table01">
		...
		<%
			if(rset.getString("file_path") != null) { // 파일이 존재한다면 
				%>
				<tr>
					<td class="firstTd">파일</td>
					<td class="secondTd" colspan=3><i class="fa fa-paperclip"></i><%=rset.getString("file_path").substring(7) %></td>
				</tr>
				<%
			}
		%>		
