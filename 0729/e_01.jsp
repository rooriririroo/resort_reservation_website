...
<body style="text-align:center; ">
	<!-- 헤더 -->
	<h3 style="color:#6b639f; margin-top:50px; font-size:25px; font-weight:bold;">리조트 소식</h3>
...
<%	
	...
	try{
		...
		// 2. 행 출력
		rset = stmt.executeQuery("select * from gongji order by id desc;"); // execute : 모든 쿼리 실행 가능
		while(rset.next()) {		
			lineCnt++; // 행 번호 카운트
...
%>
		<tr>
			<td class="idTd" style="background-color:lavender;">
				<a href="gongji/gongji_view.jsp?id=<%=rset.getInt("id") %>"><%=rset.getInt("id") %></a>
			</td>
			<td class="titleTd" style="text-align:left; text-overflow:ellipsis; overflow:hidden; white-space:nowrap;">
				<a href="gongji/gongji_view.jsp?id=<%=rset.getInt("id") %> "><nobr><%=title %></nobr></a>
<%
	// 파일이 존재한다면 표시
...
%>
		<div style="text-align:center; margin-left:600px;">
			<input type="button" class="btn" value="작성" onclick="location.href='gongji/gongji_insert.jsp';"/>
		</div>
<%	}%>
   <!--  페이지바 출력부 -->
...
%>
         ...
         <a href='e_01.jsp?from=1&cnt=<%=cntPT%>'> &lt;&lt; </a>
<%
   } else {
%>
         <!--  특수문자 << -->
         <!-- 다른 페이지일 때는 이전 페이지바로 이동 -->
         <a href='e_01.jsp?from=<%=(fromPT/(cntPT*10)-1)*(cntPT*10)+1%>&cnt=<%=cntPT%>'> &lt;&lt; </a>
<%
   }
...
%>
      ...
      <a 
<% } %>
      href='e_01.jsp?from=<%=(i*cntPT-(cntPT-1))%>&cnt=<%=cntPT%>'><%=(i)%></a>
<%   
...
%>
      ...
      <a href='e_01.jsp?from=<%=(lineCntTotal-lineCntTotal%cntPT-cntPT-1)%>&cnt=<%=cntPT%>'> &gt;&gt; </a>
<% } else {
%>
      <a href='e_01.jsp?from=<%=(lineCntTotal-lineCntTotal%cntPT+1)%>&cnt=<%=cntPT%>'> &gt;&gt; </a>
<%
}
